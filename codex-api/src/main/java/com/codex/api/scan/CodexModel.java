...
public class CodexModel {
     ...
     * 注解对象
     */
    private transient CodexApi codex;
    /**
    ...
    public CodexModel(Class<?> codexClazz) {
        ...
        this.codex = codexClazz.getAnnotation(CodexApi.class);
    ...
    public void addFieldModel(CodexFieldModel fieldModel) {
        this.codexFieldModels.add(fieldModel);
        this.codexFieldMap.put(fieldModel.getFieldName(), fieldModel);
        if (fieldModel.getCodexField() != null && fieldModel.getCodexField().search().value()) {
            this.searchFieldModels.add(fieldModel);
        }
    }
