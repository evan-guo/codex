...
import com.codex.api.constant.CodexConstants;
...
public class ApiPathBuilder {
    ...
    private List<Map.Entry<String, PathItem>> pathItemList;

    public static void build(OpenAPI openApi, CodexModel codexModel, Schema schema, List<Map.Entry<String, PathItem>> pathItemList) {
        ...
        for (Map.Entry<String, PathItem> pathItemEntry : pathItemList) {
            ...
            openApi.getPaths().put(replacePathVariable, pathItem);
        }
     ...
     * 设置Schema各个字段的标题
     */
    private static void setSchemaTitle(Schema schema, CodexModel codexModel) {
        if (schema != null) {
            // 获取Schema所有属性
            ...
            for (CodexFieldModel fieldModel : codexFieldModels) {
                ...
                if (fieldModel.getCodexField() != null) {
    ...
    private static void createSchemas(Map<String, Schema> schemas, Schema schema, CodexModel codexModel) {
        // 查询条件
     ...
     * 创建查询条件
     */
    private static Schema createQuerySchema(Schema schema, CodexModel codexModel) {
        Map<String, Schema> properties = schema.getProperties();
     ...
     * 创建分页查询条件
     */
    private static Schema createPageQuerySchema(Schema schema, CodexModel codexModel) {
     ...
     * 创建单个查询响应对象
     */
    private static Schema createResponseSchema(Schema schema, CodexModel codexModel) {
        Schema pageSchema = new Schema();
     ...
     * 创建分页查询响应对象
     */
    private static Schema createPageResponseSchema(Schema schema, CodexModel codexModel) {
        Schema pageSchema = new Schema();
     ...
     * 创建接口路径PathItem
     */
    private static PathItem createPathItem(Map.Entry<String, PathItem> pathItemEntry, CodexModel codexModel) {
        CodexApi codex = codexModel.getCodex();
        // 深拷贝
    ...
    private static Operation getOperation(PathItem copyPathItem) {
        return Optional.ofNullable(copyPathItem.getGet())
                .or(() -> Optional.ofNullable(copyPathItem.getPost()))
                .or(() -> Optional.ofNullable(copyPathItem.getPut()))
                .or(() -> Optional.ofNullable(copyPathItem.getDelete()))
                .orElse(null);
    }
     ...
     * 校验接口权限
     */
    private static boolean checkPathPower(Power power, Operation operation) {
        if (!power.enable()) {
            return true;
        }
     ...
     * 重写ResponseBody
     */
    private static void setResponseBody(Operation operation, CodexModel codexModel) {
